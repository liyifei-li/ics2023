...
//RV32I Base Instruction Set
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc; s->dnpc = cpu.pc + imm; IFDEF(CONFIG_ITRACE, if (funccnt) jal_ftrace(s->snpc - 4, s->dnpc, rd)));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & 0xfffffffe; IFDEF(CONFIG_ITRACE, if (funccnt == 0) jalr_ftrace(s->snpc - 4, s->dnpc, INSTPAT_INST(s), rd)));
