...
//RV32I Base Instruction Set
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = (src1 + imm) & 0xfffffffe; IFDEF(CONFIG_ITRACE, jalr_ftrace(s->snpc - 4, s->dnpc, INSTPAT_INST(s), rd));if (s->dnpc == 0) printf("%x %x\n", src1, imm));
